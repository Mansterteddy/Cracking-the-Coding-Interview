...
//但是这样会很浪费空间，于是引出了第二种方法，构造两个栈，一个栈存放元素，一个栈存放当前最小值，每次push元素时，也要查看元素此时，是否比最小值栈的栈顶小，如果小于或者等于，push进去。
//同时pop时也要查看当前pop元素和最小栈栈顶元素的关系，如果相等，也要pop。此时最小栈的栈顶元素仍然是当前栈最小值。
