...
//在大数据情况下，快速查找和匹配。可以采用暴力匹配的法，先匹配根，然后再依次匹配左右子树。存在特殊情况，比如多一个最右子节点，那么查看先序遍历和中序遍历后是否始终包含它。要遍历完整棵树才可以确认查找完毕。
//先序遍历和中序遍历可以确定一棵树，中序遍历和后序遍历可以确定一棵树，然而先序遍历和后序遍历无法确定一棵树，因为无法分开先序遍历根节点后面的部分，无法知道哪部分是左子树，哪部分是右子树，进而无法递归处理。
//如果先序、中序、后序遍历小树和大树，小树得到的序列始终是大树的子序列，则为子树。这种方法不对，可以通过特殊的构造，使得第一棵树中包含三条同先序、中序、后序遍历的路径，但是事实上属于不同的树。
